...
public class DancingLinks {
	...
	// this.masterColumn = new ColumnNode(-1); // the root is used as an entry-way
	// to the linked list i.e. we access
	// // the list
	// // through the root
	// // create the column heads
	// ColumnNode curColumn = this.masterColumn;
	// for (int col = 0; col < matrix[0].length; col++) // getting the column heads
	// from the sparse matrix and filling
	// // in the information about the
	// // constraints. We iterate for all the column heads, thus going through all
	// the
	...
	// // curColumn.info = id; // the information about the column is set to the new
	// // column
	// curColumn.column = curColumn;
	// }
	// curColumn.right = this.masterColumn; // making the list circular i.e. the
	// right-most ColumnHead is linked to the
	// // root
	...
	// if (matrix[row][col] == 1) // i.e. if the sparse matrix element has a 1 i.e.
	// there is a clue here i.e.
	...
	// curColumn = (ColumnNode) curColumn.right;
	// }
	// // return this.masterColumn; // We've made the doubly-linked list; we return
	// the
	// // root of the list
	// }
	...
	private void createDancingLinks(int[][] coverMatrix) {
		...
		for (int j = 0; j < this.numberOfColumnNodes; j++) {
			...
			// this.masterColumn = (ColumnNode)
			// this.masterColumn.linkHorizontally(columnNode);
